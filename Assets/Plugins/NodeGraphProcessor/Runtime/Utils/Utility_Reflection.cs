...
{
    public static class UtilityRefelection
    {
        private static readonly Dictionary<string, Assembly> AssemblyCache = new();
        private static readonly Dictionary<string, Type> FullNameTypeCache = new();
        private static readonly List<Type> AllTypeCache = new();
        private static readonly Dictionary<Type, IEnumerable<Type>> ChildrenTypeCache = new();
        ...
        public static IEnumerable<Type> GetChildrenTypes(Type baseType)
        {
            ...
            {
                foreach (var item in childrenTypes) yield return item;
                yield break;
            }

            ChildrenTypeCache[baseType] = childrenTypes = BuildCache(baseType);
            foreach (var type in childrenTypes) yield return type;
        }
        ...
        public static Type GetType(string fullName, string assemblyString)
        {
            ...
            foreach (var tempType in assembly.GetTypes()) FullNameTypeCache[tempType.FullName] = tempType;
            if (FullNameTypeCache.TryGetValue(fullName, out type))
                return type;
            return null;
        }
        ...
        private static readonly Dictionary<Type, List<FieldInfo>> _typeFieldInfoCache = new();
        ...
        {
            if (_typeFieldInfoCache.TryGetValue(type, out var fieldInfos))
                return fieldInfos;
